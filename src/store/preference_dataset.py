...
data backend the whole system writes preference signals into; today that is the
explicit profile edits, and the 👍/👎 feedback loop will write its votes here
too (see the event schema below) without any refactor.
...
``profile_add`` / ``profile_remove``  *(implemented now)*
    ...
    explicit preference signal.
        ``{"kind": "author"|"keyword"|"topic"|"conference"|"seed", "value": str}``
...
``vote``  *(written by the 👍/👎 feedback loop; see TelegramPoller)*
    A 👍/👎 on a shown paper — the strongest, explicitly *labelled* signal.
        ``{"signal": "up"|"down"|"none", "canonical_key": str, "score": float,
           "breakdown": dict, "text": str}``
...
``impression``  *(schema only — NOT logged here yet; the feedback loop will)*
    A paper that was *shown* to the user (the exposure half of the vote signal,
    needed to model what was seen-but-not-voted).
        ``{"canonical_key": str, "score": float, "breakdown": dict,
           "route": "alert"|"digest"}``

Only ``profile_add`` / ``profile_remove`` are produced today. ``vote`` and
``impression`` are documented so the feedback loop writes into this same store,
with the same ``log()`` API, without changing the schema or this class.
"""
...
class PreferenceDataset:
    def __init__(self, path: str = "data/preferences.jsonl") -> None:
        self.path = Path(path)
    ...
    # ---- reading -----------------------------------------------------------
    def events(self, types: list[str] | None = None) -> list[dict]:
...
class ProfileListener:
    ...
    def __call__(self, action: str, kind: str, value: str) -> None:
        ...
        self._dataset.log({"type": event_type, "kind": kind, "value": value})
